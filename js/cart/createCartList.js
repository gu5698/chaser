...
function doFirst() {
    ...
    for (var key in items) {
        ...
        if (itemInfo == null) {
            ...
            var trItemList = '';
            // 賦予每個row的id
            trItemList += '<div class="container products"><div class = "row"><div class = "col-12 offset-md-3 col-md-8 list_border">';
            trItemList += '<center style="margin: 4rem;"> <h1> 購物車現在是空的唷！ 快去商城看看吧！ </h1></center></div> </div> </div>';
            // 當同id的row不存在時置入頁面
...
function actions() {
    ...
    $(".next").click(function () {
        if ($.trim($("#coupon").val()) != "" && $(".discount").val() == "") {
            alert("如果輸入優惠券記得點選 使用優惠券 做使用唷！");
        } else {
            var itemString = storage.getItem('addItemList');
            var items = itemString.substr(0, itemString.length - 2).split(', ');
            for (var key in items) {
                storage.removeItem(items[key]);
                storage['addItemList'] = storage['addItemList'].replace(items[key] + ', ', '');
            }
            $("form").submit();
        };
    });
