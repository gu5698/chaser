...
function submenuCreate() {
    ...
    for (var key in items) {
        ...
        if (itemInfo == null) {
            ...
            trItemList.className = 'shoplist bfdn';
            //建立商品名稱和--第一個div
...
function createSubmenuList(itemKey, itemInfo) {
    ...
    //建立商品名稱和刪除按鈕--第三個td
    var tdQty = document.createElement('div');
    tdQty.className = 'product-qty';
    tdQty.innerText = itemQty;
    trItemList.appendChild(tdQty);

    //建立商品名稱和刪除按鈕--第三個td
    var tdTotal = document.createElement('div');
    tdTotal.className = 'product-price';
    tdTotal.innerHTML = itemTotal + "<span style='font-size:12px; padding: 0 0 0 8px;'>USD</span>";
